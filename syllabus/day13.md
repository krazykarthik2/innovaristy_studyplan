...
### 📌 TL;DR
Interface with a physical 4-DoF robot manipulator. Write a custom python node that opens a serial communication link to the Dobot Magician, queries joint coordinates, and sends motor speed and PTP target commands.
...
* **Serial Comm in Linux (`/dev/tty*`)**
  ...
  * Fix permission issues by adding your user account to the `dialout` group (so you can read/write to the USB port):
    ...
    ```
    *(Note: You must log out and log back in for this to apply).*
* **Writing the Serial Driver Node in Python**
  ...
  * The node needs two main functions:
    ...
    2. **Subscriber Callbacks**: Listen for target coordinates (`geometry_msgs/msg/Point`), parse them into Dobot-specific serial packets, and write them to the USB serial port.
* **Control Space Modes**
